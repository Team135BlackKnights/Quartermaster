...
string subsystem_name(DB db, Subsystem_id id)
{
	auto q = q1<string>(
		...
		"FROM subsystem_info "
		"WHERE (id) IN "
		"(SELECT MAX(id) FROM subsystem_info WHERE subsystem_id=" +
			escape(id) + ") "
						 "AND valid");
	if (q.size() == 0)
		return "No subsystem name found";
	if (q.size() > 1)
	{
		PRINT(id);
		PRINT(q);
...
string part_name(DB db, Part_id id)
{
	auto q = q1<string>(
		...
		"FROM part_info "
		"WHERE (id) IN "
		"(SELECT MAX(id) FROM part_info WHERE part_id=" +
			escape(id) + ") "
						 "AND valid");
	if (q.empty())
		return "No part name found";
	// PRINT(q);
	assert(q.size() == 1);
	return q[0];
}
...
void sortable_labels(ostream &o, Request const &page, vector<Label> const &labels)
{
	...
	for (auto label : labels)
	{
		...
		auto p2 = page;
		std::visit([&](auto &x)
				   { x.sort_by=label.text; x.sort_order="asc"; },
				   p2);
		auto p3 = page;
		std::visit([&](auto &x)
				   { x.sort_by=label.text; x.sort_order="desc"; },
				   p3);
		o << label << " ";
		o << link(p2, "<i class=\"fas fa-sort-up\"></i>");
		o << link(p3, "<i class=\"fas fa-sort-down\"></i>");
		o << "</th>";
	}
	o << "</tr>";
}
...
BOM_item bom_data(DB db, optional<Subsystem_id> subsystem)
{
	...
	for (auto elem : parts)
	{
		...
		auto official_cost = [=]() -> Decimal
		{
			...
			if (category != Bom_category::STANDARD)
				return 0;
			return qty * price;
		}();
	...
	f = [&](optional<Subsystem_id> a) -> BOM_item
	{
		...
		else
		{
			...
			r.official_cost = sum(mapf([](auto x)
									   { return x.official_cost; },
									   r.children));
			r.weight = [=]()
			{

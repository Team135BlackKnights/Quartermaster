	...
	while (getline(ss, pair, ';'))
	{
		...
		if (eq != string::npos)
		{
			...
			while (!key.empty() && key[0] == ' ')
				key.erase(0, 1); // trim left spaces
			cookies[key] = value;
		}
	}
	return cookies;
}
std::string current_user(DB db)
{
	try
	{
		...
		std::string question = "SELECT username FROM sessions WHERE session_token = '" + session_token + "'";
		// Create Con
		auto rows = query(db, question);
		// log to Docker
		//  log rows to Docker
		if (rows.empty() || rows[0].empty() || !rows[0][0].has_value())
			return "no_user";
		return *rows[0][0]; // valid user
	}
...
void inner(ostream &o, Subsystem_new_data const &a, DB db)
{
	...
	if (a.parent)
	{
		auto q = "INSERT INTO subsystem_info ("
				 ...
				 "parent "
				 ") VALUES (" +
				 escape(id) + "," + escape(current_user(db)) + ","
															   "now(),"
															   "1," +
				 escape(a.parent) + ")";
		run_cmd(db, q);
...
#define X(A, B)                             \
	if ("" #B == string("part_number"))     \
		v |= part_entry(db, a.parent, a.B); \
	else                                    \
		v |= pair<string, string>("" #B, escape(a.B));
...
#undef X
	...
	make_page(
		...
		string() + "<form>"
				   ...
				   "<input type=\"hidden\" name=\"subsystem_id\" value=\"" +
			escape(a.id) + "\">" +
			after_done() + input_table([=]()
									   {
			vector<Input> r;
#define X(A, B) r |= show_input(db, "" #B, current.B);
			SUBSYSTEM_DATA(X)
#undef X
			...
			"<br><input type=\"submit\">" +
			"</form>" + link([=]()
							 { BOM r; r.subsystem=a.id; return r; }(), "BOM") +
			" " + link([=]()
					   { Subsystem_duplicate r; r.subsystem=a.id; return r; }(), "Duplicate") +
			h2("Overview") + [=]()
			{
			stringstream ss;
			...
			return ss.str(); }() + "<table border><tr>" + th("Name") + th("Status") + "</tr>" + indent_sub_table(db, 0, a.id, {}) + "</table>" + parts_of_subsystem(db, a, a.id) + subsystems_of_subsystem(db, a, a.id) + completion_est(db, a.id) + h2("History") + make_table(a, {
//"edit_date","edit_user","id","name","subsystem_id","valid","parent","prefix"
#define X(A, B) "" #B,
																																																																		SUBSYSTEM_INFO_ROW(X)
#undef X
																																																																	},
																																																																 query(db, "SELECT " + join(",", info_col_names) + " FROM subsystem_info WHERE subsystem_id=" + escape(a.id))));
}
...
optional<Subsystem_id> get_parent(DB db, Subsystem_id id)
{
	auto q = qm<optional<Subsystem_id>>(
		...
		"WHERE "
		"valid "
		"AND id IN (SELECT MAX(id) FROM subsystem_info WHERE subsystem_id=" +
			escape(id) + ")");
	if (q.size() == 0)
		return std::nullopt;
	assert(q.size() == 1);

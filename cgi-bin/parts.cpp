...
{
	return h2("Current state") + subsystem_state_count(db, page) + subsystem_machine_count(db, page) + to_do(db, page) + done(db, page);
}
...
string history_sum(DB db)
{
	...
	auto q = qm<
		PART_INFO_ROW(A_COMMA)
			Dummy>(
		db,
		"SELECT " PART_INFO_ROW(B_STR_COMMA) "0 FROM part_info ORDER BY id");
	vector<Label> labels;
...
void inner(std::ostream &o, By_user const &a, DB db)
{
	...
	make_page(
		...
		links + show_table_user(db, a, "subsystem_info", a.user) + show_table_user(db, a, "part_info", a.user) + show_table_user(db, a, "meeting_info", a.user));
}
...
string by_machine(DB db, Request const &page, Machine const &a)
{
	auto qq = qm<Part_state, Subsystem_id, Part_id, unsigned, Decimal>(
		...
		"AND machine='" +
			as_string(a) + "' "
						   "ORDER BY part_state");
	return "<a name=\"" + as_string(a) + "\"></a>" + h2(as_string(a)) + as_table(db, page, vector<Label>{"Status", "Subsystem", "Part", "Qty", "Time"}, qq) + "Total time:" + as_string(sum(get_col<4>(qq)));
}
...
void inner(std::ostream &o, Machines const &a, DB db)
{
	...
	make_page(
		o,
		"Machines",
		h2("Contents") + join("<br>", mapf([=](auto x)
										   { return "<a href=\"#" + as_string(x) + "\">" + as_string(x) + "</a>"; }, machines())) +
			join("", mapf([=](auto x)
						  { return by_machine(db, a, x); }, machines())));
}
...
{
#define X(A)                       \
	if (holds_alternative<A>(req)) \
		return inner(o, get<A>(req), db);
	PAGES(X)
	X(Error)
#undef X
	Error page;
...
void alter_column(DB db, Table_name table, pair<string, Column_type> c1, pair<string, Column_type> c2)
{
	...
	if (e1 && e2)
	{
		...
		if (to_substitute.size())
		{
			run_cmd(
				db,
				"ALTER TABLE " + table + " MODIFY " + name1 + " enum(" + join(",", MAP(escape, old_set | new_set)) + ")");
